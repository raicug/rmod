...
public:
	...
	c_hl2mp_player_anim_state *create_anim_state()
	{
		...
		if (!create_anim_state)
			throw std::runtime_error("Failed to create anim state");
	...
	void update_client_side_animations()
	{
		...
		if (!update_client_side_animations)
			throw std::runtime_error("Failed to update client side anims");
	...
	void get_eye_position(c_vector &eye_position)
	{
		...
		if (!get_eye_position)
			throw std::runtime_error("Failed to get eye position");
	...
	c_base_combat_weapon *get_active_weapon()
	{
		...
		if (!get_active_weapon)
			throw std::runtime_error("Failed to get active weapon");
	...
	void invalidate_bone_cache()
	{
		...
		if (!invalidate_bone_cache)
			throw std::runtime_error("Failed to invalidate bone cache");
	...
	void set_abs_origin(c_vector &origin)
	{
		...
		if (!set_abs_origin)
			throw std::runtime_error("Failed to set ABS origin");
	...
	void set_abs_angles(q_angle &angles)
	{
		...
		if (!set_abs_angles)
			throw std::runtime_error("Failed to set ABS angles");
