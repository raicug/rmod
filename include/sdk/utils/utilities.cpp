...
const char* utilities::host_cleanup_con_var_string_value(const char* str)
{
	...
	if (!host_cleanup_con_var_string_value)
		throw std::runtime_error("Failed to get host cleanup conv var string value");
...
float utilities::get_fov(const q_angle& from, const q_angle& to)
{
	...
	dot = std::clamp(dot, -1.0f, 1.0f); // Prevent NaNs from acos

	return math::rad2deg(acosf(dot));
}

bool utilities::screen_transform(const c_vector& in, c_vector* out)
{
	const D3DMATRIX& world_matrix = raicu::globals::settings::world_to_screen;
...
bool utilities::get_entity_box(c_base_entity* entity, box_t& box)
{
	...
	for (int i = 1; i < 8; ++i)
	{
		...
		top    = std::max<float>(top, screen_points[i].y);
		right  = std::max<float>(right, screen_points[i].x);
		bottom = std::min<float>(bottom, screen_points[i].y);
	}
