...
#include "../../../gui/framework/helpers/custom.h"
...
bool ImGui::SliderScalar( const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags ) {
    ...
    float v_cur = *(const float*)p_data;
    float percent = (v_cur - v_min) / (v_max - v_min);
    float width = frame_bb.GetWidth() * ((v_cur - v_min) / (v_max - v_min));

    value->second = ImLerp(value->second, percent * frame_bb.GetWidth(), 0.03f);
    ...
    window->DrawList->AddRectFilled(
        frame_bb.Min,
        ImVec2(frame_bb.Min.x + value->second, frame_bb.Max.y),
        custom.get_accent_color(1.0f),
        2
    );
    RenderText( total_bb.Min, label );
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    {
        RenderNavHighlight(frame_bb, id);
        RenderFrame(frame_bb.Min, frame_bb.Max, GetColorU32(ImVec4(21.0f/255.0f, 20.0f/255.0f, 27.0f/255.0f, 1.0f)), true, 2);
    }
...
#define IM_TRUNC(x) static_cast<float>(static_cast<int>(x))
